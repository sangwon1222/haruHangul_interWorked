<template>
  <div class="wrap-step03">
    <timerCheck />
    <div class="page-header">
      <!-- <router-link :to="{ name: 'main' }" class="btn-back" /> -->
      <div class="btn-back" @click="goMainPage" />
      <h2>
        <span class="tit">{{ stepName }}</span>
      </h2>
    </div>
    <ul class="day-list" @click="playclickSnd">
      <li v-for="(f, index) in curriculum" :key="index">
        <!-- 종합(깨단한글)일 경우 === 마지막 -->
        <div v-if="index >= curriculum.length - 1">
        ...
        <div v-else>
          <!-- 기본일 경우 -->
          <div v-if="f.curriculum.length >= 5">
            <div v-if="f.day < dayOfStudy">
              <basicComponent
                :state="{ active: 'defualt', info: f, day: dayOfStudy }"
              />
    ...
    async getList() {
      try {
        const { data } = await this.$axios.get(
          '/learning/hangul/contents/step03'
        )
        // this.stepName = data.result.name
        this.stepName = '복잡한 모음'
